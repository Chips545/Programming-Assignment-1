...
Game.prototype.handleMouseDown = function(e) {
	...
	switch (Tools.current) {
		case Tools.SELECT:
            ...
            this.tileMap[pos.row] = (this.tileMap[pos.row] === undefined) ? [] : this.tileMap[pos.row];
			this.tileMap[pos.row][pos.col] = 1;
			break;
		case Tools.MOVE:
	...
	for (var row = startRow; row < rowCount; row++) {
		for (var col = startCol; col < colCount; col++) {
			...
			if (this.tileMap[row] != null && this.tileMap[row][col] != null) {
				...
				xpos -= ((building.width * this.zoomHelper.level) / 2) - (tileWidth / 2);
